...
def _build_root_loader(data_cfg, root_size, batch_size, seed):
    """FLTrust's clean root set — shared with the training-time defense ensemble
    (``server/defense_ensemble.py``) so both bootstrap trust from the same samples."""
    from data.mnist_loader import build_root_loader
    return build_root_loader(data_dir=data_cfg.get("data_dir", "./data/mnist_raw"),
                             root_size=root_size, batch_size=batch_size, seed=seed)
